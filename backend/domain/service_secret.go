...
func (s *service) SetThresholdParticipants(
	...
	return s.database.WithTx(ctx, func(database db.Database) error {
		actor, err := database.SelectSecretForManager(ctx, secretID, userID)
		if err != nil {
			return err
		}
		if actor.Threshold == 0 ||
			(actor.Role != models.ThresholdRoleOwner && actor.Role != models.ThresholdRoleMaintainer) {
			return errors.New("threshold access cannot be managed")
		}
		current, err := database.SelectThresholdParticipantKeys(ctx, secretID)
		if err != nil {
			return err
		}
		...
		if !sameThresholdParticipants(input.ExpectedParticipants, listed) {
			return ErrThresholdParticipantsChanged
		}
		...
		if owner.UserID == 0 {
			return errors.New("threshold owner is missing")
		}
		...
		for _, participantInput := range input.Participants {
			...
			if actor.Role == models.ThresholdRoleMaintainer {
				...
				if (participantInput.Role == models.ThresholdRoleMaintainer &&
					...
					(exists && existing.Role == models.ThresholdRoleMaintainer &&
						participantInput.Role != models.ThresholdRoleMaintainer) {
					return errors.New("only the owner can grant maintainer access")
				}
			}
		...
		if actor.Role == models.ThresholdRoleMaintainer {
			for _, participant := range current {
				if participant.Role == models.ThresholdRoleMaintainer {
					...
					if !exists {
						return errors.New("only the owner can remove maintainer access")
					}
				}
			}
		}
		if len(updated) < actor.Threshold {
			return errors.New("participant count cannot be lower than the threshold")
		}
		return s.rotateThresholdShares(ctx, database, actor, userID, updated, cipher)
	})
}
